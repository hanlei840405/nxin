...
public class LogService {
    ...
    private static final String SQL_TRANS = "SELECT `ID_BATCH`,\n" +
            ...
            "    `REPLAYDATE`,\n" +
            "    `LOG_FIELD`\n" +
            "FROM `log_etl_transform` WHERE ID_BATCH = (SELECT MAX(ID_BATCH) FROM `log_etl_transform` WHERE CHANNEL_ID in (:channelIds) GROUP BY CHANNEL_ID) \n";
    ...
    public String jobLog(String channelId) {
        Map<String, Object> record = jdbcTemplate.queryForMap("SELECT log_field FROM log_etl_job WHERE channel_id=?", channelId);
        return (String) record.get("log_field");
    }
...
//            List<JobLogVo> jobLogVos = this.jobLog(logChannelIds);
//            jobLogVos.forEach(log -> builder.append(log.getLogField()));
//        } else {
//            List<TransformLogVo> transformLogs = this.transformLog(logChannelIds);
